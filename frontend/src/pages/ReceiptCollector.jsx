    ...
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 md:space-y-6 page-enter">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-cyber-muted font-mono">Delivery acknowledgment timestamps and RTT records</p>
        </div>
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="text-xs font-mono text-cyber-muted">
            Total: <span className="text-cyber-accent">{total}</span>
          </div>
        </div>
      </div>

      <form onSubmit={handleFilter} className="glass-card p-4 flex flex-wrap items-center gap-3">
        <Filter size={14} className="text-cyber-muted flex-shrink-0" />
        <input
          className="cyber-input flex-1 min-w-[220px]"
        ...
        {filterProbe && (
          <button type="button" onClick={() => { setFilterProbe(''); load(1, '') }} className="btn-danger py-1.5 px-3 text-xs">
            CLR
          </button>
        )}
      </form>
      ...
      <div className="glass-card p-4 md:p-5">
        <div className="overflow-x-auto">
          <table className="cyber-table min-w-[720px]">
            <thead>
              <tr>
                <th>Receipt ID</th>
                <th>Probe ID</th>
            ...
            <tbody>
              ...
              {!loading && receipts.map((r) => (
                <tr key={r.id}>
                  ...
                  <td><span className="tag tag-purple">{r.ack_source}</span></td>
                  <td><RTTBadge rtt={r.rtt_ms} /></td>
                  <td>{r.raw_payload?.sim ? <span className="tag tag-yellow">SIM</span> : <span className="tag tag-cyan">LIVE</span>}</td>
                </tr>
              ))}
              {!loading && receipts.length === 0 && (
                <tr>
                  <td colSpan={6} className="text-center text-cyber-muted py-10 font-mono text-sm">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mt-4 text-xs font-mono text-cyber-muted">
          <span className="break-words">Page {page} · {receipts.length} records shown of {total}</span>
          <div className="flex gap-2">
            <button
              disabled={page === 1}
              onClick={() => { setPage((p) => p - 1); load(page - 1, filterProbe) }}
              className="btn-cyber py-1 px-3 text-xs disabled:opacity-30"
            >
              PREV
            </button>
            <button
      ...
      <div className="flex flex-wrap items-center gap-2 md:gap-4 text-xs font-mono text-cyber-muted">
