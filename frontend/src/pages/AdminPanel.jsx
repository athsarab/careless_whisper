...
import { Shield, Plus, Trash2, Download, UserPlus, Smartphone, Users, Play, Square } from 'lucide-react'
...
function Modal({ open, onClose, children }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md mx-4 glass-card p-6 glow-cyan">
        {children}
      </div>
    </div>
  )
...
export default function AdminPanel() {
  ...
  const exportCSV = async () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = 'sdra_export.csv'; a.click()
    setExporting('')
  }
  ...
  const exportPDF = async () => {
    ...
    const url = URL.createObjectURL(res.data)
    const a = document.createElement('a'); a.href = url; a.download = 'sdra_report.pdf'; a.click()
    setExporting('')
  }
  ...
  return (
    <div className="flex-1 overflow-y-auto p-6 space-y-6 page-enter">
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <button id="export-csv-btn" onClick={exportCSV} disabled={exporting === 'csv'}
          className="btn-green flex items-center gap-2 text-sm">
          <Download size={14} /> {exporting === 'csv' ? 'EXPORTING...' : 'EXPORT CSV'}
        </button>
        <button id="export-pdf-btn" onClick={exportPDF} disabled={exporting === 'pdf'}
          className="btn-green flex items-center gap-2 text-sm">
          <Download size={14} /> {exporting === 'pdf' ? 'EXPORTING...' : 'EXPORT PDF'}
        </button>
        <button id="admin-toggle-sim" onClick={toggleSim}
          className={`flex items-center gap-2 text-sm btn-cyber ${simRunning ? 'btn-danger' : 'btn-green'}`}>
          ...
          <tbody>
            {devices.map(d => (
              <tr key={d.id}>
                ...
                <td>{d.consent_verified
                  ? <span className="tag tag-green">VERIFIED</span>
                  : <span className="tag tag-red">PENDING</span>}</td>
                <td className="text-cyber-muted">{new Date(d.created_at).toLocaleDateString()}</td>
                <td>
                  <button onClick={() => removeDevice(d.id, d.label || d.phone_number)}
          ...
          <tbody>
            {users.map(u => (
              <tr key={u.id}>
                ...
                <td>{u.role === 'admin'
                  ? <span className="tag tag-cyan">ADMIN</span>
                  : <span className="tag tag-purple">RESEARCHER</span>}</td>
                <td className="text-cyber-muted">{new Date(u.created_at).toLocaleDateString()}</td>
                <td>
                  <button onClick={() => removeUser(u.id, u.username)}
                    className="text-cyber-muted hover:text-cyber-red transition-colors">
                    <Trash2 size={14} />
                  </button>
                </td>
      ...
      <Modal open={deviceModal} onClose={() => setDeviceModal(false)}>
        ...
        <form onSubmit={addDevice} className="space-y-4">
          <div>
            ...
            <input className="cyber-input" placeholder="+1234567890" required
              value={deviceForm.phone_number}
              onChange={e => setDeviceForm(f => ({ ...f, phone_number: e.target.value }))} />
          </div>
          <div>
            <label className="text-xs font-mono text-cyber-muted mb-1 block uppercase tracking-wider">Label (optional)</label>
            <input className="cyber-input" placeholder="e.g. Test Device Alpha"
              value={deviceForm.label}
              onChange={e => setDeviceForm(f => ({ ...f, label: e.target.value }))} />
          </div>
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={deviceForm.consent_verified}
              onChange={e => setDeviceForm(f => ({ ...f, consent_verified: e.target.checked }))}
